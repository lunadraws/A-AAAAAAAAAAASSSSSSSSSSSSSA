    ...
    if (!message.member.voice.channel){
      ...
      return message.channel.send(
        new MessageEmbed()
          ...
          .setFooter(`${currentQueue.tracks.length > 5 ? `And ${currentQueue.tracks.length - 5} other songs...` : `In the playlist ${currentQueue.tracks.length} song(S)...`} | \©️${new Date().getFullYear()} HorizonGame`)
      );
    };
  }
};
