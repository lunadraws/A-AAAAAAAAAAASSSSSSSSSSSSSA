...
module.exports = {
  ...
  run: async function ( client, message, args ) {
    ...
    if (!query){
      ...
      return message.channel.send('\\<a:a_ERROR:828230829687046155> Veuillez inclure l\'utilisateur à trouver sur Anilist!');
    };
    ...
    if (response.errors){
      ...
      if (response.errors[0].status === 404){
        err = `\\<a:a_ERROR:828230829687046155> Je ne trouve pas **${query}** sur Anilist!`;
      } else if (response.errors.some(x => x.status >= 500)){
        err = `\\<a:a_ERROR:828230829687046155> Anilist n'a pas pu être atteint pour le moment! Veuillez réessayer plus tard. [err ${response.errors[0].status}]`;
      } else if (response.errors.some(x => x.status >= 400)){
        err =`\`<a:a_ERROR:828230829687046155> CLIENT_ERR \`: HorizonGame a tenté d\'envoyer une requête invalidée à AniList. Veuillez contacter mon développeur pour corriger ce bug.`;
      } else {
        err = `\\<a:a_ERROR:828230829687046155> Un problème est survenu. Veuillez réessayer plus tard`;
      };
