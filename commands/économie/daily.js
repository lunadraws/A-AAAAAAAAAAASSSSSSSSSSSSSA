...
module.exports = {
  ...
  run: (client, message) => profile.findById(message.author.id, async (err,doc) => {
    ...
    if (err){
      ...
      const supporter = await client.guilds.cache.get('833599070684708865').members.fetch(message.author.id).then(() => true).catch(() => false)
      ...
      .then(() => message.channel.send([
        ...
        hasvoted === false ? `\n\\⚠️ **Vote rewards available**: Vote now to receive additional rewards! -> <https://top.gg/bot/688407554904162365/vote>` : hasvoted === null ? '\n\\⚠️ Could not contact top.gg for vote update' : ''
      ].join('')))
