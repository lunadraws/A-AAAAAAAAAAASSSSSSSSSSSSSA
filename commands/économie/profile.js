...
module.exports = {
  ...
  run: async function (client, message, [member = '']){
    ...
    if (member.user.bot){
      ...
      return message.channel.send(`\\❌ Les robots ne peuvent pas gagner d'XP!`);
    };
    ...
    return profile.findOne({ _id: member.id }, async (err, doc) => {
      ...
      if (!doc || !doc.data.xp.some(x => x.id === message.guild.id)){
        return message.channel.send(`\\❌ **${member.user.tag}** n'a pas encore commencé à gagner de l'XP sur ce serveur!`)
      };
      ...
      const color = doc.data.profile.color || 'rgb(183,182,255)'
      ...
      const def = await loadImage(doc.data.profile.background || 'https://i.imgur.com/sSb2IgQ.jpeg');
      ...
      if (!emblem){
        ...
        ctx.textAlign = 'center'
        ctx.fillText('', 660 , 469, 150)
        ctx.fillText('', 660, 500, 150)
      } else {
        ctx.shadowBlur = 10;
        ctx.shadowOffsetX = 10;
